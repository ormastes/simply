...
`test_result`, `test_command`, `test_exit`, `test_output_sha256`,
`failure_phase`, `sspec_paths`, `sspec_review=basic-static`, `checked_at`,
`valid_until`, and `follow_up`. The command output digest and
...
The only admitted result tuples are `verified`/`0`/`none` and
`failed`/nonzero/(`simple-version` or `project-test`). Expired otherwise-valid
receipts retain their audited follow-up but never retain a verified claim.
