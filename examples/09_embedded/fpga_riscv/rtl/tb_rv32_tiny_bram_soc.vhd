-- tb_rv32_tiny_bram_soc: GHDL rehearsal of the EXACT synthesizable BRAM SoC
-- (rv32_bram_soc: rv32_exec_core_axi + BRAM slave + UART capture + obs port)
-- before burning a Vivado run. Only the Vivado top's device primitives
...
--   RV32_BRAMSOC_BOOT_DONE reached=TEST_PASSED   -- live UART tap marker chain
--   RV32_BRAMSOC_OBS_MAGIC_OK / STATUS_OK        -- obs port sane, pass flag set
--   RV32_BRAMSOC_OBS_UART_MATCH bytes=<n>        -- capture buffer readback is
--                                                   byte-identical to the live
--                                                   UART transcript
--
-- Generics are overridable from ghdl -r via -gTINY_RAM_WORDS=... etc. so the
-- same tb rehearses both the 512KB and the shrunk 256KB budgets.
library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;
use std.env.all;

entity tb_rv32_tiny_bram_soc is
  generic (
    TINY_RAM_WORDS   : natural := 131072;
    TINY_RDISK_WORDS : natural := 262144;
    TINY_UARTBUF_WORDS : natural := 2048
  );
end entity tb_rv32_tiny_bram_soc;

architecture sim of tb_rv32_tiny_bram_soc is
...
begin
  ...
  process
  begin
    report "RV32_BRAMSOC_BUDGET ram_bytes=" & integer'image(TINY_RAM_WORDS * 4)
  ...
  begin
    if rising_edge(clk) then
      if rst = '0' and debug_uart_valid = '1' then
        ...
        if ch = LF then
          report "RV32_BRAMSOC_UART_LINE: " & lbuf(1 to llen) severity note;
          if llen >= 11 and (lbuf(1 to 11) = "TEST PASSED" or lbuf(1 to 11) = "TEST FAILED") then
            if lbuf(1 to 11) = "TEST PASSED" then
              report "RV32_BRAMSOC_BOOT_DONE reached=TEST_PASSED" severity note;
            else
              report "RV32_BRAMSOC_BOOT_DONE reached=TEST_FAILED" severity note;
            end if;
            done <= true;
          end if;
          llen := 0;
  ...
  -- Observation-port exercise once the boot finished (or timeout).
  process
    ...
    begin
      ...
      assert obs_resp(63 downto 48) = x"A55A"
        report "RV32_BRAMSOC_OBS_BAD_SIG resp=" & to_hstring(obs_resp) severity note;
      assert obs_resp(47 downto 32) = cmd(15 downto 0)
        report "RV32_BRAMSOC_OBS_BAD_ECHO resp=" & to_hstring(obs_resp) severity note;
  ...
  begin
    ...
    if not done then
      report "RV32_BRAMSOC_BOOT_STUCK pc=0x" & to_hstring(debug_pc) severity note;
    end if;
    ...
    if d = x"51F0B007" then
      report "RV32_BRAMSOC_OBS_MAGIC_OK" severity note;
    else
      report "RV32_BRAMSOC_OBS_MAGIC_BAD data=" & to_hstring(d) severity note;
    end if;

    obs_do(x"00000001", d);
    report "RV32_BRAMSOC_OBS_STATUS pass=" & std_logic'image(d(31))
      & " fail=" & std_logic'image(d(30))
      & " count=" & integer'image(to_integer(unsigned(d(23 downto 0)))) severity note;
    if d(31) = '1' and d(30) = '0' then
      report "RV32_BRAMSOC_OBS_STATUS_OK" severity note;
    end if;
    ...
    if mism = 0 and nbytes > 0 then
      report "RV32_BRAMSOC_OBS_UART_MATCH bytes=" & integer'image(nbytes) severity note;
    else
      report "RV32_BRAMSOC_OBS_UART_MISMATCH bytes=" & integer'image(nbytes)
